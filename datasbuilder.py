...
#not work in a thread, reseason unknown
...
def getWord(word):
    ...
    text=re.subn(r"(?P<num>\s[0-9]+\s)",'<span style="color: #ff0000; background-color: #c0c0c0; font-weight: bold; font-size: 120%;">\g<num></span>',text)[0]
    
    res=ur"(?P<example>\*[^\u4e00-\u9fa5]+[\u4e00-\u9fa5\s()（）\[\],.:、]+)"
    text=re.subn(res,'<span style="background-color:#ceffce;font-size:14px;">\g<example></span>',text)[0]

    res=ur"(?P<example>\sn\s|\sv\s|\sadj\s)"
    text=re.subn(res,'<span style="color:white;background-color:black;font-weight:bold;font-size:18px;">\g<example></span>',text)[0]
...
def youdao(word):
    ...
    result=pattern.search(content.read())
    if result is None: return "youdao failed to find '"+word+"'"
    result=result.group()
...
def text2html(text):
    ...
    t2=text[t1+1:].find('/')
    if t2==-1 : return text
    text="<div style='font-size:17px'>"+text+""
    return text
